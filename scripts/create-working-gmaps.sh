...
echo "🔧 Criando versão funcional do GMaps Extractor..."

# Criar o arquivo correto
...
export class GMapExtractorService {
  ...
  async scrollAndExtractResults(page: puppeteer.Page): Promise<ExtractedResult[]> {
    ...
    while (scrollAttempts < maxScrollAttempts) {
      // Executar extração no contexto do browser
      const newResults = await page.evaluate(() => {
        // Todo este código roda no contexto do browser, onde document e HTMLElement existem
        ...
        return items.map((item) => {
          try {
            ...
            const infoElements = Array.from(item.querySelectorAll("div.fontBodyMedium span"))
            const infoTexts = infoElements
              .map(el => el.textContent?.trim())
              .filter(text => text && text.length > 0)

            // Tentar encontrar avaliação
            ...
            // Tentar encontrar número de avaliações
            const reviewElement = item.querySelector("span.fontBodySmall")
            ...
            return {
              name,
              address: infoTexts[0] || "",
              category: infoTexts[1] || "",
              rating,
              reviewCount,
              phone: infoTexts.find(text => text.match(/[\+$$$$\d\s\-]{10,}/)) || "",
              website: infoTexts.find(text => text.includes("www") || text.includes("http")) || ""
            }
          } catch (error) {
...
if [ $? -eq 0 ]; then
    ...
    echo "🧪 Testando rotas..."
    curl -s http://localhost:5000/api/gmaps-extractor/campaigns || echo "Rota não respondeu"
    
    kill $SERVER_PID 2>/dev/null
    
    echo "✅ GMaps Extractor configurado com sucesso!"
    echo "📋 Próximos passos:"
    echo "1. cd backend && npm run dev"
    echo "2. Teste o frontend em http://localhost:3000/dashboard/gmaps-extractor"
else
