...
echo "🔧 Corrigindo os últimos 2 erros do GMaps Extractor..."

# Criar o arquivo correto sem NENHUM código DOM fora do page.evaluate()
...
export class GMapExtractorService {
  ...
  async scrollAndExtractResults(page: puppeteer.Page): Promise<ExtractedResult[]> {
    ...
    while (scrollAttempts < maxScrollAttempts) {
      // TODO O CÓDIGO DOM ESTÁ AQUI DENTRO DO page.evaluate()
        ...
        return items
          .map((item: any) => {
            ...
            return {
              ...
              rating,
              reviewCount,
              phone: info.find((i: string) => i && (i.includes("+") || /\d+/.test(i))) || "",
              website: info.find((i: string) => i && (i.includes("www") || i.includes("http"))) || "",
            }
          })
      ...
      // ROLAR TAMBÉM ESTÁ DENTRO DO page.evaluate()
      await page.evaluate(() => {
        const feed = document.querySelector("div[role='feed']") as any
        if (feed) {
