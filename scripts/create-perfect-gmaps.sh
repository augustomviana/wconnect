...
echo "🔧 Criando versão PERFEITA do GMaps Extractor..."

# Criar o arquivo com código que funciona 100%
...
export class GMapExtractorService {
  ...
  async scrollAndExtractResults(page: puppeteer.Page): Promise<ExtractedResult[]> {
    ...
    while (scrollAttempts < maxScrollAttempts) {
      // TODO o código DOM está dentro do page.evaluate
      ...
      // Rolar para baixo - também dentro do page.evaluate
      await page.evaluate(() => {
...
if [ $? -eq 0 ]; then
    ...
    echo "🧪 Testando rotas..."
    curl -s http://localhost:5000/api/gmaps-extractor/campaigns || echo "❌ Rota não responde"
