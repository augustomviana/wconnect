...
echo "🔧 Corrigindo erros de TypeScript no GMaps Extractor..."
...
export class GMapExtractorService {
  ...
  async scrollAndExtractResults(page: puppeteer.Page): Promise<ExtractedResult[]> {
    ...
    while (scrollAttempts < maxScrollAttempts) {
      // @ts-ignore - O código abaixo será executado no contexto do navegador via Puppeteer
...
if [ $? -eq 0 ]; then
    ...
    echo "🚀 GMaps Extractor está pronto para uso!"
