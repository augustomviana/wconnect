...
echo "🔧 Criando versão FINAL funcional do GMaps Extractor..."
...
export class GMapExtractorService {
        ...
        return items.map((item: any) => {
          try {
            ...
            return {
              ...
              rating,
              reviewCount,
              phone: info.find((i: any) => i && (i.includes("+") || /\d+/.test(i))) || "",
              website: info.find((i: any) => i && (i.includes("www") || i.includes("http"))) || "",
            }
          } catch (error) {
            return null
          }
  ...
  async getCampaignResults(campaignId: string): Promise<any[]> {
    try {
      const result = await this.db.query(
        `SELECT * FROM gmap_results WHERE campaign_id = $1 ORDER BY created_at DESC`,
        [campaignId],
      )
      return result.rows
    } catch (error) {
